<x-app-layout>
    ...
    <h1>Actualizar Participante : {{$persona->id}}</h1>
    ...
    <form action="{{route('persona.update', $persona)}}" method="post" enctype="multipart/form-data">
    	...
    	<label>Carrera</label>
    	<input type="text" class="form-control" class="form-control" name="carrera" value="{{ old('carrera',$persona->carrera) }}">
    	...
    	<br>
    	<input type="file" class="w-full" name="foto" value="{{ old('foto',$persona->foto) }}">

       <button class="btn btn-success mt-3" type="submit">Actualizar Participante</button>
    </form>
